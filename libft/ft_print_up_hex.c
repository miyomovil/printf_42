...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/02/21 15:03:07 by antomart          #+#    #+#             */
/*   Updated: 2020/06/22 14:28:20 by antomart         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "libft.h"

void	ft_print_ulprec2_uh(unsigned long int nbr, t_env *ptr)
{
...
void	ft_print_ulprec_uh(unsigned long int nbr, t_env *ptr)
{
	ptr->pr_i = 0;
	...
	ft_print_ulprec2_uh(nbr, ptr);
}

void	ft_print_ulmin_uzer_uh(unsigned long int *nbr, int len, t_env *ptr)
{
	ptr->pr_i = 0;
...
void	ft_print_up_hex_nbr(unsigned long nbr, t_env *ptr)
{
	...
	if (nbr > 16)
	{
		ft_print_up_hex_nbr((nbr / 16), ptr);
		ft_print_up_hex_nbr((nbr % 16), ptr);
	}
	else
	{
...
void	ft_print_up_hex(unsigned long int nbr, t_env *ptr)
{
	if (ptr->prec != -1 && ft_get_atr_length(nbr) < ptr->prec)
		ft_print_ulprec_uh(nbr, ptr);
	else
	{
		if (ptr->min == 1 && ptr->prec != 0)
		{
			ft_print_up_hex_nbr(nbr, ptr);
			ft_print_ulmin_uzer_uh(&nbr, ft_get_atr_length(nbr), ptr);
		}
		else
		{
